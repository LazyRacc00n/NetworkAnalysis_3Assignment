...
- On Facebook in each experiment the infection curves are characterized by a long tail mainly because of the small probability *q*. But such behaviour is not present on those of Karate club where the simulation ends after few steps after the initial decreases. Another reason for that is the kind of structure of the network, the Facebook dataset seems to be less conductive in certain regions between the communites.
- The experiments on Facebook requires way more times steps to end up, also because of the number of nodes.
